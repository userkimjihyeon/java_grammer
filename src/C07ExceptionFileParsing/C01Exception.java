...
//        login(input);                       //⭐호출한곳 -> 예외가 여기로 전파됨.

//        ⭐CheckedException의 경우에는 예외처리를 위임받게 되면, 반드시 예외처리를 하거나, 다시 throws 해야함.
        try {
            login2(input);
...
//            CheckedException은 예외처리가 강제.
//            해당메서드 내에서 예외처리를 하든, 아니면 명시적으로 throws를 통해 호출 메서드쪽에 위임.
