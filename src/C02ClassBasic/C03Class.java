...
//        MyCalculatorInstance.sumAcc() : stack이 없으므로 sumAcc는 호출이 불가 -> 객체로 만들어서 호출해야함
        mi1.sumAcc(10);
        System.out.println(mi1.total);  //⭐직접 힙메모리영역에서 값을 조회
        mi1.sumAcc(20);
        System.out.println(mi1.total);
//        B부서 매출 : 10원, 20원
        MyCalculatorInstance mi2 = new MyCalculatorInstance();  //객체 mi2 생성 후 메서드 호출
        mi2.sumAcc(10);
        System.out.println(mi2.total);  //⭐직접 힙메모리영역에서 값을 조회
        mi2.sumAcc(20);
...
//    static이 붙으면 공유되어 부서끼리 매출이 중복됨. -> static 제거 -> 클래스는 붕어빵틀이닷~! -> 객체는 붕어빵이닷~!
